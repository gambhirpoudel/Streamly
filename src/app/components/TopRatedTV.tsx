...
import { fetchtopRatedTVs, fetchTvDetails } from '../api/tmdb';
...
const TopRatedTvs: React.FC = () => {
  ...
  useEffect(() => {
    const getTrendingTVs = async () => {
      try {
        const tvShows = await fetchtopRatedTVs();
        setTopRatedTvs(tvShows);
  ...
  return (
    <div className="relative w-full h-auto py-8 bg-gradient-to-b from-gray-900 to-black text-white overflow-hidden">
      <h1 className="text-3xl font-semibold mb-6 pl-5 text-left">Top Rated TV Shows</h1>
      <div 
